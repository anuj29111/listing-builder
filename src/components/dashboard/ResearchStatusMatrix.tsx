...
interface StatusMatrixProps {
  ...
  onCellClick?: (categoryId: string, countryId: string) => void
}
    ...
    <div className="rounded-lg border bg-card">
      <div className="p-4 border-b">
        ...
        <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
          ...
          <span className="ml-auto text-[11px] italic">Click any cell to open</span>
        </div>
      </div>
                ...
                {countries.map((country) => {
                  ...
                  return (
                    <td
                      ...
                      onClick={() => onCellClick?.(category.id, country.id)}
                    >
                      {types.length === 0 ? (
