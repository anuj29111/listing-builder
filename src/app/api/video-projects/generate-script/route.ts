...
export async function POST(request: Request) {
  try {
    ...
    // Use existing storyboard for alignment if available
    const existingStoryboard = existingProject.data?.storyboard as unknown as VideoStoryboardResult | null

    const { result, model, tokensUsed } = await generateVideoScript({
      productName,
      brand,
      categoryName,
    ...
    const upsertData = {
      listing_id,
      script: result as unknown as Record<string, unknown>,
      script_model: model,
      script_tokens_used: tokensUsed,
    ...
    if (existingProject.data) {
      const { data, error } = await supabase
        .from('lb_video_projects')
        .update({
          script: upsertData.script,
          script_model: upsertData.script_model,
          script_tokens_used: upsertData.script_tokens_used,
